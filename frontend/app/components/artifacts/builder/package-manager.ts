import { ArtifactsFetch } from "~/components/artifacts/utils/artifacts-fetch";
...
export class PackageManager {
  ...
  private async loadPackage(
    ...
    if (!current) {
      const version =
        ...
        this._dependencies[packageName] ??
        "latest";
    ...
    for (const subPackage of current.subPackages) {
      if (packagePath == subPackage.path) {
        if (!subPackage.manifest) {
          subPackage.manifest = await this.getPackageManifest(
            current.name,
            current.version,
            subPackage.path
          );
  ...
  private async getPackageManifest(
    ...
    const response = await ArtifactsFetch.fetch(
      `${basePath}${relativePath}/package.json`
    );
  ...
  private async getPackageFiles(packageName: string, version: string) {
    ...
    const response = await ArtifactsFetch.fetch(dataPath);

    if (!response.ok) {
      throw new Error(
        `Failed to fetch package data for "${packageName}@${version}"`
      );
    }
  ...
  async loadFile(context: BuilderContext, path: string) {
    ...
    if (!data) {
      const response = await ArtifactsFetch.fetch(url);

      if (!response.ok) {
        throw new Error(
          `Failed to fetch file "${packageName}@${current.version}${packagePath}"`
        );
      }
    ...
    if (!packagePath) {
      ...
      for (const subPackage of current.subPackages) {
        if (packagePath == subPackage.path) {
          ...
          if (!manifest) {
            throw new Error(
              `Manifest not found in package: ${packageName}/${subPackage.path}`
            );
          }
          ...
          if (!main) {
            throw new Error(
              `Main file not found in package: ${packageName}/${subPackage.path}`
            );
          }
