...
import { ArtifactsFetch } from "../../utils/artifacts-fetch";
...
export class ShadcnHook extends BuilderHookBase {
  ...
  override async beforeBuild(context: BuilderContext): Promise<void> {
    ...
    const files = Object.entries(context.files).filter(
      ([key]) => key.endsWith(".tsx") || key.endsWith(".jsx")
    );
    for (const [, value] of files) {
      ...
      if (
        ...
        value.includes("/hooks/use-toast")
      ) {
        enabled = true;
        break;
      }
  ...
  override async loadFile(
    ...
    if (path.startsWith(componentsPrefix) || path.startsWith(registryPrefix)) {
      ...
      // https://www.jsdelivr.com/package/npm/shadcn-ui
    ...
    } else if (path.startsWith(hooksPrefix)) {
      path = BuilderUtils.stripStart(hooksPrefix, path);
      const componentsBase = `shadcn-ui/ui@${this._version}/apps/www/registry/${theme}/hooks`;
      const url = `${BuilderConstants.CDN_BASE_GH}/${componentsBase}/${path}.ts`;
