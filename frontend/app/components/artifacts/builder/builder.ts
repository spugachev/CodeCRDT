...
import type { BuilderPreset, Files } from "../presets/types";
...
export class Builder {
  ...
  async build() {
    ...
    try {
      ...
      if (typeof error === "object") {
        ...
        const buildWarnings = (
          error as { warnings: esbuild.Message[] }
        ).warnings.map(
          (warning) =>
  ...
  async generateHtml() {
    ...
    if (buildResult) {
      const html = this._context.preset.generateHtml(
        this._context,
        buildResult,
      );
  ...
  private getVFSPlugin(): esbuild.Plugin {
    ...
    const onResolve = async (
      ...
      const basePath = BuilderUtils.getBasePath(
        BuilderUtils.resolve(args.importer, [paths, packageManager.paths]) ??
          args.importer,
      );

      let path =
        (await packageManager.processPath(preset.dependencies, args.path)) ??
        args.path;
    ...
    const onLoad = async (
      ...
      if (args.path.includes("/undefined")) {
        if (args.path.endsWith("/undefined")) {
          args.path = args.path.replace("/undefined", "/index.js");
        } else {
          args.path = args.path.replace("/undefined", "");
        }
      }
      ...
      if (
        typeof retValue === "undefined" &&
        typeof files[args.path] !== "undefined"
      ) {
      ...
      if (
        typeof retValue === "undefined" &&
        args.path.startsWith("/node_modules/")
      ) {
        const contents = await packageManager.loadFile(
          this._context,
          args.path,
        );
        if (contents) {
