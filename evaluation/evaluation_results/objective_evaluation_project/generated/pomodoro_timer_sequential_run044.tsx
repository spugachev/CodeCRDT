      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-8">
                ...
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      i < completedPomodoros % 4 ? `bg-gradient-to-r ${currentColors.primary}` : 'bg-white/20'
                    }`}
                  />
                ))}
              </div>
        ...
        >
          <Button
            onClick={toggleTimer}
            size="lg"
            className={`px-8 py-6 rounded-2xl text-lg font-semibold backdrop-blur-xl bg-gradient-to-r ${currentColors.primary} hover:scale-105 transition-all duration-300 shadow-lg ${currentColors.glow} border-0`}
          >
            {isRunning ? (
              <>
        ...
        <AnimatePresence>
          {showSettings && (
            <motion.div
              ...
              className="mt-8 backdrop-blur-2xl bg-white/5 p-8 rounded-3xl border border-white/10 shadow-2xl"
            >
              <h3 className="text-2xl font-bold text-white mb-6">Timer Settings</h3>
              <div className="grid grid-cols-3 gap-6">
                {(['work', 'shortBreak', 'longBreak'] as TimerMode[]).map((m) => (
                  <div key={m} className="flex flex-col">
                    <label className="text-slate-400 mb-2 text-sm">
                      {m === 'work' ? 'Work' : m === 'shortBreak' ? 'Short Break' : 'Long Break'}
                    </label>
                    <input
                      type="number"
                      ...
                      className="px-4 py-3 rounded-xl backdrop-blur-xl bg-white/5 border border-white/10 text-white text-center text-lg focus:outline-none focus:ring-2 focus:ring-white/20"
                      min="1"
                      max="60"
                    />
                  </div>
        ...
        <motion.div
          ...
          className="mt-12 backdrop-blur-xl bg-white/5 px-8 py-4 rounded-2xl border border-white/10"
        >
          <div className="text-center">
            <div className="text-3xl font-bold text-white">{completedPomodoros}</div>
            <div className="text-slate-400 text-sm mt-1">Pomodoros Completed</div>
          </div>
        </motion.div>
      </div>
        ...
        {showCompletion && (
          ...
          >
            <motion.div
              ...
              className={`backdrop-blur-2xl bg-gradient-to-br ${currentColors.primary} p-12 rounded-3xl shadow-2xl border border-white/20`}
            >
              <motion.div
                animate={{
                  scale: [1, 1.2, 1],
              ...
              <h2 className="text-4xl font-bold text-white text-center mb-2">
                {mode === 'work' ? 'Great Work!' : 'Break Complete!'}
              </h2>
