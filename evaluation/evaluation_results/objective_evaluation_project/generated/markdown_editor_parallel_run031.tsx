...
const MarkdownComponents = {
  ...
  code: ({ inline, className, children, ...props }: any) => {
    ...
    return !inline && language ? (
      ...
      <code className="bg-[#2d2d2d] text-pink-400 px-2 py-0.5 rounded text-sm font-mono border border-gray-700">
        {children}
      </code>
    );
  },
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-blue-500 bg-[#2d2d2d] pl-4 py-3 my-4 italic text-gray-300 rounded-r">
      {children}
    </blockquote>
  ),
  hr: () => (
  ...
  th: ({ children }: any) => (
    <th className="px-4 py-2 text-left text-gray-100 font-semibold border-b border-gray-700">
      {children}
    </th>
  ),
  td: ({ children }: any) => (
