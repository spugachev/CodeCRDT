...
const MarkdownComponents = {
  ...
  code: ({ inline, className, children, ...props }: any) => {
    ...
    return !inline && match ? (
      ...
      <code className="bg-[#1e1e1e] text-pink-400 px-2 py-0.5 rounded text-sm font-mono border border-[#30363d]" {...props}>
        {children}
      </code>
    );
  },
...
export default function MarkdownEditor() {
  ...
  const handleToolbarAction = useCallback((action: string) => {
    ...
    if (action.includes('text') || action.includes('item')) {
      // For patterns like **text**, *text*, `code`, - item, 1. item
      const parts = action.split(selectedText.length > 0 ? selectedText : 'text');
      if (selectedText.length > 0) {
        newText = parts[0] + selectedText + parts[1];
        cursorOffset = parts[0].length + selectedText.length + parts[1].length;
      } else {
        newText = action;
