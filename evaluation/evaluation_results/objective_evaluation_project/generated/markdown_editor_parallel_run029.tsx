  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h3: ({ node, ...props }) => (
          <h3 className="text-2xl font-semibold mb-2 mt-4 text-pink-400" {...props} />
        ),
        h4: ({ node, ...props }) => (
          <h4 className="text-xl font-semibold mb-2 mt-3 text-cyan-400" {...props} />
        ),
        p: ({ node, ...props }) => (
        ...
        code: ({ node, inline, className, children, ...props }: any) => {
          ...
          return !inline && language ? (
            <div className="my-4 rounded-lg overflow-hidden border border-[#30363d]">
              <div className="bg-[#1e1e1e] px-4 py-2 text-xs text-gray-400 font-mono border-b border-[#30363d]">
                {language}
              </div>
              <SyntaxHighlighter
                style={vscDarkPlus}
            ...
            <code
              className="bg-[#1e1e1e] text-orange-300 px-1.5 py-0.5 rounded text-sm font-mono border border-[#30363d]"
              {...props}
            >
              {children}
            </code>
        ...
        blockquote: ({ node, ...props }) => (
          <blockquote
            className="border-l-4 border-blue-500 pl-4 py-2 my-4 bg-[#161b22] text-gray-400 italic"
            {...props}
          />
        ),
        hr: ({ node, ...props }) => (
        ...
        th: ({ node, ...props }) => (
          <th className="border border-[#30363d] px-4 py-2 text-left font-semibold text-gray-300" {...props} />
        ),
        td: ({ node, ...props }) => (
          <td className="border border-[#30363d] px-4 py-2 text-gray-400" {...props} />
        ),
      }}
    >
      {content}
