import { useState, useCallback } from 'react';import { useRef } from 'react';
...
const MarkdownComponents = {
  ...
  h3: ({ children }: any) => (
    <h3 className="text-2xl font-semibold mb-3 mt-4 text-green-400">
      {children}
    </h3>
  ),
  h4: ({ children }: any) => (
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-blue-500 bg-[#2d2d2d] pl-4 py-2 my-4 italic text-gray-400">
      {children}
    </blockquote>
  ),
  hr: () => (
...
export default function MarkdownEditor() {
  ...
  const handleToolbarAction = useCallback((action: string) => {
    ...
    if (action.includes('**text**')) {
      ...
      const beforeLine = markdown.substring(0, lineStart);
      const afterLine = markdown.substring(lineStart);
      setMarkdown(beforeLine + '- ' + afterLine);
      setTimeout(() => {
      ...
      const beforeLine = markdown.substring(0, lineStart);
      const afterLine = markdown.substring(lineStart);
      setMarkdown(beforeLine + '1. ' + afterLine);
      setTimeout(() => {
        textarea.selectionStart = textarea.selectionEnd = lineStart + 3;
        textarea.focus();
      }, 0);
      return;
    }
    ...
    const updatedMarkdown = before + newText + after;
