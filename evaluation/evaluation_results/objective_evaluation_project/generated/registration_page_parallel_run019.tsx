...
import { Mail, Lock, User, Eye, EyeOff, CheckCircle2, AlertCircle } from 'lucide-react';import { Loader2 } from 'lucide-react';
...
export default function RegistrationPage() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 relative overflow-hidden">
      ...
      />
      <motion.div
        className="absolute bottom-20 left-1/2 w-72 h-72 bg-orange-300 rounded-full mix-blend-multiply filter blur-xl opacity-70"
        animate={{
          x: [0, 50, 0],
          y: [0, -50, 0],
          scale: [1, 1.1, 1],
                    ...
                    >
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-medium ${
                          ...
                          formData.password.length < 10 ? 'text-yellow-500' :
                          /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/.test(formData.password) ? 'text-green-500' :
                          'text-blue-500'
                        }`}>
                          {formData.password.length < 6 ? 'Weak' :
                           formData.password.length < 8 ? 'Fair' :
                           formData.password.length < 10 ? 'Good' :
                           /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/.test(formData.password) ? 'Strong' :
                           'Very Good'}
                        </span>
                      </div>
                        ...
                        {[1, 2, 3, 4].map((bar) => (
                          <motion.div
                            ...
                            className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
                              formData.password.length < 6 && bar === 1 ? 'bg-red-500' :
                              formData.password.length >= 6 && formData.password.length < 8 && bar <= 2 ? 'bg-orange-500' :
                              formData.password.length >= 8 && formData.password.length < 10 && bar <= 3 ? 'bg-yellow-500' :
                              formData.password.length >= 10 && bar <= 4 ? 'bg-green-500' :
                              'bg-gray-200'
                            }`}
                          />
                        ))}
