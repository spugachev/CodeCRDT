...
const MarkdownComponents = {
  code({ node, inline, className, children, ...props }: any) {
    ...
    return !inline && language ? (
      ...
      <code
        className="bg-[#2d2d2d] text-orange-400 px-1.5 py-0.5 rounded text-sm font-mono border border-gray-700"
        {...props}
      >
        {children}
      </code>
  ...
  h3: ({ children }: any) => (
    <h3 className="text-2xl font-semibold mb-2 mt-4 text-green-400">
      {children}
    </h3>
  ),
  h4: ({ children }: any) => (
    <h4 className="text-xl font-semibold mb-2 mt-3 text-yellow-400">
      {children}
    </h4>
  ),
  h5: ({ children }: any) => (
    <h5 className="text-lg font-semibold mb-2 mt-3 text-pink-400">
      {children}
    </h5>
  ),
  h6: ({ children }: any) => (
    <h6 className="text-base font-semibold mb-2 mt-3 text-cyan-400">
      {children}
    </h6>
  ),
  p: ({ children }: any) => (
  ...
  th: ({ children }: any) => (
    <th className="px-4 py-2 text-left text-sm font-semibold text-blue-400 border border-gray-700">
      {children}
    </th>
  ),
  td: ({ children }: any) => (
  ...
  em: ({ children }: any) => (
    <em className="italic text-pink-300">
      {children}
    </em>
  ),
};
                ...
                >
                  ...
                  >
                    ...
                    <motion.span
                      ...
                      className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-xs bg-gray-900 px-2 py-1 rounded whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                    >
                      {button.label}
                    </motion.span>
                  </Button>
