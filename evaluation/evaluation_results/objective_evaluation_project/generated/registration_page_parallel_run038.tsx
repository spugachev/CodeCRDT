...
export default function RegistrationPage() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 relative overflow-hidden">
      ...
      <motion.div
        className="absolute top-1/4 left-1/4 w-64 h-64 bg-purple-400 rounded-full mix-blend-multiply filter blur-xl opacity-70"
        animate={{
          x: [0, 100, 0],
          y: [0, -100, 0],
          scale: [1, 1.2, 1],
      ...
      />
      <motion.div
        className="absolute top-1/3 right-1/4 w-72 h-72 bg-pink-400 rounded-full mix-blend-multiply filter blur-xl opacity-70"
        animate={{
          x: [0, -100, 0],
          y: [0, 100, 0],
          scale: [1, 1.3, 1],
      ...
      />
      <motion.div
        className="absolute bottom-1/4 left-1/3 w-80 h-80 bg-orange-300 rounded-full mix-blend-multiply filter blur-xl opacity-70"
        animate={{
          x: [0, 100, 0],
          y: [0, 50, 0],
          scale: [1, 1.1, 1],
            ...
            {isSuccess ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.password && (
                    ...
                    >
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-medium ${
                          calculatePasswordStrength(formData.password).label === 'Weak' ? 'text-red-500' :
                          calculatePasswordStrength(formData.password).label === 'Fair' ? 'text-orange-500' :
                          calculatePasswordStrength(formData.password).label === 'Good' ? 'text-yellow-500' :
                          'text-green-500'
                        }`}>
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4].map((bar) => {
                          const { strength, color } = calculatePasswordStrength(formData.password);
                          const isActive = strength >= bar * 25;
                          return (
                            <motion.div
                              key={bar}
                              initial={{ scaleX: 0 }}
                            ...
                            >
                              <motion.div
                                ...
                                className={`h-full ${isActive ? color : 'bg-gray-200'}`}
                              />
                            </motion.div>
                          );
                        })}
                  ...
                  </parameter><xcrdt_code_output crdtPosition="AJL6wdAK+jkB">
